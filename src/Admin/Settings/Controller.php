...
use Greys\WooCommerce\Korea\Admin\Settings\General;
use Greys\WooCommerce\Korea\Admin\Settings\SEP;
use Greys\WooCommerce\Korea\Admin\Settings\Support;
use Greys\WooCommerce\Korea\Admin\Settings\Addons;
use Greys\WooCommerce\Korea\Admin\Settings\Advanced;
...
class Controller {
	...
	public static function get_settings_pages() {
		if ( empty( self::$settings ) ) {
			...
			self::$settings = apply_filters( 'woocommerce_korea_get_settings_pages', $settings );
		}
	...
	public static function save() {
		...
		check_admin_referer( 'woocommerce-korea-settings' );

		// Trigger actions.
		do_action( 'woocommerce_korea_settings_save_' . $current_tab );
		do_action( 'woocommerce_korea_update_options_' . $current_tab );
		do_action( 'woocommerce_korea_update_options' );

		self::add_message( __( 'Your settings have been saved.', 'korea-for-woocommerce' ) );

		do_action( 'woocommerce_korea_settings_saved' );
	}
	...
	public static function output() {
		...
		do_action( 'woocommerce_korea_settings_start' );

		//wp_enqueue_script( 'wc-korea-admin', plugins_url( 'assets/js/admin.js', MAIN_FILE ), array(), VERSION, true );
		wp_enqueue_style( 'wc-korea-admin', plugins_url( 'assets/css/admin.css', MAIN_FILE ), array(), VERSION );

		// Get tabs for the settings page.
		$tabs = apply_filters( 'woocommerce_korea_settings_tabs_array', array() );

		include BASENAME . '/includes/admin/views/html-admin-settings.php';
	}
	...
	public static function output_fields( $options ) {
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				default:
					do_action( 'woocommerce_korea_admin_field_' . $value['type'], $value );
					break;
			}
		}
	}
	...
	public static function save_fields( $options, $data = null ) {
		...
		foreach ( $options as $option ) {
			 ...
			 */
			$value = apply_filters( 'woocommerce_korea_admin_settings_sanitize_option', $value, $option, $raw_value );

			/**
			 * Sanitize the value of an option by option name.
			 */
			$value = apply_filters( "woocommerce_korea_admin_settings_sanitize_option_$option_name", $value, $option, $raw_value );
			 ...
			 */
			do_action( 'woocommerce_korea_update_option', $option );
		}
