describe( 'Korea for WooCommerce General Settings', function () {
	...
	it( 'can update postcode settings', function () {
		cy.visit(
			'/wp-admin/admin.php?page=wc-settings&tab=integration&section=korea'
		);
		...
		cy.get( '#woocommerce_korea_postcode_yn' ).should( 'be.checked' );
	} );

	it( 'displays postcode in checkout page', function () {} );
} );
